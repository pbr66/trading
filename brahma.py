...
load_dotenv()
TIDB_DATABASE_URL = os.getenv("TIDB_URL")
...
# Assets mapped to your TiDB tables
assets = {
    'NASDAQ': 'nasdaq', 
    'S&P 500': 'sp500',
    'Dow Jones': 'dow', 
...
table_to_load = assets[sel_asset]
...
# --- 3. DATA ENGINE (TiDB + RE-SAMPLING LOGIC) ---

@st.cache_resource
def get_engine():
    """Creates the SQLAlchemy engine once and caches it."""
    if not TIDB_DATABASE_URL:
        return None
...
def load_news_data(filepath="news.xlsx"):
    """Reads the multi-tab news.xlsx file (Keeping this local for Macro events)"""
    ...
    try:
        ...
        for sheet in xls.sheet_names:
            ...
            temp['Event'] = str(sheet).upper() 
            
            time_col = 'time' if 'time' in temp.columns else None
            temp['Time_str'] = temp[time_col].astype(str) if time_col else None
...
@st.cache_data
def load_data(table_name, tf, asset_name):
    engine = get_engine()
    if engine is None:
        st.error("Missing TIDB_URL in .env file.")
        return None, None
        
    try:
        with st.spinner(f"Loading {asset_name} from TiDB..."):
        ...
        if 'dt' in df.columns:
            ...
            st.error(f"Could not find a date/time column in TiDB table '{table_name}'. Available columns: {list(df.columns)}")
            return None, None

        req_cols = [c for c in ['dt', 'open', 'high', 'low', 'close'] if c in df.columns]
        df = df.dropna(subset=req_cols).sort_values('dt')

        df['tz_offset'] = '+01' 
        df['dt_utc'] = pd.to_datetime(df['dt'], utc=True, errors='coerce')
...
df_i, df_d = load_data(table_to_load, sel_tf, sel_asset)

if df_d is not None:
    sess = st.sidebar.pills("Trading Hours", ['ETH', 'RTH'], default='RTH', key='sess')
    levels = st.sidebar.pills("Levels", ["Globex High", "Globex Low", "Globex Close", "RTH High", "RTH Low", "RTH Close", "ON High", "ON Low"], default=["Globex High", "Globex Low", "RTH Close"], selection_mode="multi", key='levels')
    ...
    st.sidebar.divider()
    backtest_mode = st.sidebar.toggle("🔬 Backtest Mode", value=False, key='backtest_mode')
    ...
    with st.sidebar.expander("🛠️ Measured Move Tool", expanded=False):
        ...
        if mm_on:
            ...
            if mm_calc_type == "Fractions (%)":
                ...
                try: mm_levels_list = [float(x.strip()) for x in mm_mults_str.split(",")]
                except: mm_levels_list = [0, 0.5, 1] 
            else:
                mm_pts_str = st.text_input("Point Target Levels", "10, 20, 30, 40, 50", key='mm_pts_str')
                try: mm_pts_list = [float(x.strip()) for x in mm_pts_str.split(",")]
                except: mm_pts_list = [10, 20, 50]
    ...
    if f_15m_2_pat != 'Any': df_f = df_f[df_f['f_15m_2_pat'] == f_15m_2_pat]
        
    if f_5m_1_len_opt == 'Less': df_f = df_f[df_f['f_5m_1_len'] < len_low]
    elif f_5m_1_len_opt == 'Between': df_f = df_f[(df_f['f_5m_1_len'] >= len_low) & (df_f['f_5m_1_len'] <= len_high)]
    elif f_5m_1_len_opt == 'Greater': df_f = df_f[df_f['f_5m_1_len'] > len_high]
    ...
    if pill_dates:
        ...
        if sess == 'RTH': pd_df_full = pd_df_full[pd_df_full['is_rth']]
        ...
        with slider_cont:
            if backtest_mode and not pd_df_full.empty:
                ...
                if 'last_bt_date' not in st.session_state or st.session_state.last_bt_date != sd:
                    st.session_state.bar_idx = 1; st.session_state.last_bt_date = sd
        ...
        pad_time = pd.Timedelta(minutes=tf_mins_map.get(sel_tf, 5) * 2)
            ...
            with cols[0]:
                if not pd_df_full.empty and day_info is not None:
                    ...
                    if pd.notna(info['rc']) and pd.notna(info['ro']) and pd.notna(info['ec']) and pd.notna(info['eo']):
                        ...
                        st.markdown(f"""<div style="background-color: {badge_bg}; color: {'#0d652d' if pts_diff > 0 else '#a50e0e'}; padding: 4px 10px; border-radius: 4px; display: inline-block; font-weight: bold; font-size: 14px; margin-bottom: 5px;">Result: {sign}{pts_diff:.1f} pts</div>""", unsafe_allow_html=True)
                    else: st.markdown("<div style='margin-bottom: 30px;'></div>", unsafe_allow_html=True)

                    n_ev = info.get('News_Event', np.nan)
                    news_html = f" | <span style='color:#d93025;'><b>🚨 NEWS: {n_ev}</b></span>" if pd.notna(n_ev) and n_ev != '' else ""
                    st.markdown(f"""<div style="font-size: 0.85em; padding: 8px; background-color: #e8f0fe; border-radius: 6px; margin-bottom: 10px; color: #1967d2; border: 1px solid #d2e3fc;"><b>{sel_asset}</b> | {sd} ({info['DayName']})<br>Open: <b>{info['on_rel']} ON</b> | Gap: {info['gap']:.2f}%<br>1st 5m: {info['f_5m_1']} | 2nd 15m: {info['f_15m_2']}{news_html}</div>""", unsafe_allow_html=True)

                    pd_df_plot = pd_df_full.copy()
                    if backtest_mode and current_time_limit is not None: pd_df_plot.loc[pd_df_plot['dt'] > current_time_limit, ['open', 'high', 'low', 'close']] = np.nan
                    ...
                    fig.add_trace(go.Scatter(x=pd_df_plot['dt'], y=[0.03]*len(pd_df_plot), yaxis='y2', mode='markers', marker=dict(color='rgba(0,0,0,0)', size=1), customdata=custom_data, hovertemplate="<b>Time:</b> %{x|%H:%M} &nbsp;&nbsp;|&nbsp;&nbsp; <b>O:</b> %{customdata[0]:,.2f} &nbsp;&nbsp; <b>H:</b> %{customdata[1]:,.2f} &nbsp;&nbsp; <b>L:</b> %{customdata[2]:,.2f} &nbsp;&nbsp; <b>C:</b> %{customdata[3]:,.2f}<extra></extra>", showlegend=False))

                    rb = pd_df_slice[pd_df_slice['is_rth']].copy()
                    rb['n'] = np.arange(1, len(rb) + 1)
                    
                    # MEASURED MOVE TOOL 
                    rb_tool = pd_df_full[pd_df_full['is_rth']].copy()
                    rb_tool['n'] = np.arange(1, len(rb_tool) + 1)
                    if mm_on and not rb_tool.empty:
                        def get_p(b_num, p_type):
                            row = rb_tool[rb_tool['n'] == b_num]
                            return row.iloc[0][p_type.lower()] if not row.empty else None

                        pt1, pt2 = None, None
                        if mm_mode == "1 Bar":
                            pt1, pt2 = get_p(mm_b1, mm_a1), get_p(mm_b1, mm_a2)
                        elif mm_mode == "2 Bars":
                            pt1, pt2 = get_p(mm_b1, mm_a1), get_p(mm_b2, mm_a2)
                        elif mm_mode == "Range":
                            s_n, e_n = min(mm_b1, mm_b2), max(mm_b1, mm_b2)
                            sub_df = rb_tool[(rb_tool['n'] >= s_n) & (rb_tool['n'] <= e_n)]
                            if not sub_df.empty: pt1, pt2 = sub_df['high'].max(), sub_df['low'].min()

                        if pt1 is not None and pt2 is not None:
                            if mm_calc_type == "Fractions (%)":
                                if pt1 == pt2: fig.add_hline(y=pt1, line_dash="solid", line_width=2, line_color="blue", annotation_text=" 0 / 1")
                                else:
                                    for m in mm_levels_list:
                                        lvl = pt1 + ((pt2 - pt1) * m)
                                        l_d, l_w, l_c = ("solid", 2, "blue") if m in [0, 1] else (("dash", 1, "blue") if 0 < m < 1 else ("dot", 1, "rgba(128,128,128,0.7)"))
                                        fig.add_hline(y=lvl, line_dash=l_d, line_width=l_w, line_color=l_c, annotation_text=f" {m:g} ({lvl:.2f})")
                            else:
                                fig.add_hline(y=pt1, line_dash="solid", line_width=2, line_color="red", annotation_text=f" Base ({pt1:.2f})")
                                for p in mm_pts_list:
                                    lvl = pt1 + p
                                    fig.add_hline(y=lvl, line_dash="solid", line_width=1, line_color="blue" if p>0 else "purple", annotation_text=f" {lvl:.2f} ({'+' if p>0 else ''}{p:g} pts)")

                    if sess == 'ETH' and pd.notna(info['on_h']) and pd.notna(info['on_l']):
                        fig.add_shape(type="rect", x0=pd.to_datetime(f"{sd} 00:00:00"), y0=info['on_l'], x1=pd.to_datetime(f"{sd} {'08:00:00' if sel_asset in ['DAX', 'FTSE'] else '12:00:00'}"), y1=info['on_h'], line=dict(color="RoyalBlue", width=1, dash="dot"), fillcolor="LightSkyBlue", opacity=0.15, layer="below")
                    if show_50_pct and not pd_df_slice.empty:
                        fig.add_trace(go.Scatter(x=pd_df_slice['dt'], y=(pd_df_slice['high'] + pd_df_slice['low']) / 2, mode='markers', marker=dict(symbol='line-ew', size=14, color='rgba(255, 0, 0, 0.35)', line=dict(width=3, color='rgba(255, 0, 0, 0.35)')), showlegend=False, hoverinfo='skip'))
                    if pat != 'Any':
                        t_bars, y_v, m_s = pd.DataFrame(), None, 'star'
                        ...
                        if not t_bars.empty: fig.add_trace(go.Scatter(x=t_bars['dt'], y=y_v, mode='markers', marker=dict(symbol=m_s, size=10, color='black'), hoverinfo='skip', showlegend=False))
                    ...
                    if pd.notna(n_ev) and pd.notna(n_time) and str(n_time).strip() not in ['None', 'nan']:
                        try: fig.add_vline(x=pd.to_datetime(f"{sd} {str(n_time).replace('T', ' ').strip()}"), line_width=2, line_dash="dash", line_color="rgba(217, 48, 37, 0.7)", annotation_text=f" 🚨 {n_ev}", annotation_font_color="red")
                        except: pass
                    ...
                    fig.update_layout(title=dict(text=f"<span style='font-size:12px; color:gray;'>(Fullscreen Hotkeys: Shift + ➡️/⬅️ to change dates)</span>", x=0.01, y=0.98), yaxis2=dict(range=[0, 1], fixedrange=True, visible=False, overlaying='y'), height=chart_height, xaxis_rangeslider_visible=False, template="plotly_white", margin=dict(t=40, b=0, l=10, r=10), dragmode='pan', hovermode='x', hoverdistance=-1, spikedistance=-1, uirevision=sd, hoverlabel=dict(bgcolor="rgba(255, 255, 255, 0.9)", font_size=13, font_family="Arial", bordercolor="rgba(0, 0, 0, 0.2)"))
                    st.plotly_chart(fig, use_container_width=True, config={'scrollZoom': True, 'displayModeBar': True})
                else: st.warning(f"No valid intraday data found for {sd} within the selected hours.")

            # --- COLUMN 2+: COMPARE ASSETS ---
                    ...
                    with cols[i+1]:
                        ...
                        if c_df_i is not None and sd in c_df_d['Date'].values:
                            ...
                            if pd.notna(c_info['rc']) and pd.notna(c_info['ro']) and pd.notna(c_info['ec']) and pd.notna(c_info['eo']):
                                ...
                                st.markdown(f"""<div style="background-color: {'#e6f4ea' if c_pts_diff > 0 else '#fce8e6'}; color: {'#0d652d' if c_pts_diff > 0 else '#a50e0e'}; padding: 4px 10px; border-radius: 4px; display: inline-block; font-weight: bold; font-size: 14px; margin-bottom: 5px;">Result: {"+" if c_pts_diff > 0 else ""}{c_pts_diff:.1f} pts</div>""", unsafe_allow_html=True)
                            else: st.markdown("<div style='margin-bottom: 30px;'></div>", unsafe_allow_html=True)
                            ...
                            st.markdown(f"""<div style="font-size: 0.85em; padding: 8px; background-color: #f3f4f6; border-radius: 6px; margin-bottom: 10px; color: #3c4043; border: 1px solid #e8eaed;"><b>{c_asset}</b> | {sd}<br>Open: <b>{c_info['on_rel']} ON</b> | Gap: {c_info['gap']:.2f}%<br>1st 5m: {c_info['f_5m_1']}{c_news_html}</div>""", unsafe_allow_html=True)

                            if sess == 'RTH': c_pd_df_full = c_pd_df_full[c_pd_df_full['is_rth']]
                            
                            if not c_pd_df_full.empty:
                                c_pd_df_plot = c_pd_df_full.copy()
                                if backtest_mode and current_time_limit is not None: c_pd_df_plot.loc[c_pd_df_plot['dt'] > current_time_limit, ['open', 'high', 'low', 'close']] = np.nan
                                ...
                                c_fig.add_trace(go.Scatter(x=c_pd_df_plot['dt'], y=[0.05]*len(c_pd_df_plot), yaxis='y2', mode='markers', marker=dict(color='rgba(0,0,0,0)', size=1), customdata=c_custom_data, hovertemplate="<b>Time:</b> %{x|%H:%M} &nbsp;&nbsp;|&nbsp;&nbsp; <b>O:</b> %{customdata[0]:,.2f} &nbsp;&nbsp; <b>H:</b> %{customdata[1]:,.2f} &nbsp;&nbsp; <b>L:</b> %{customdata[2]:,.2f} &nbsp;&nbsp; <b>C:</b> %{customdata[3]:,.2f}<extra></extra>", showlegend=False))
                                ...
                                # MEASURED MOVE TOOL SYNC
                                c_rb_tool = c_pd_df_full[c_pd_df_full['is_rth']].copy()
                                c_rb_tool['n'] = np.arange(1, len(c_rb_tool) + 1)
                                if mm_on and not c_rb_tool.empty:
                                    def get_c_p(b_num, p_type):
                                        row = c_rb_tool[c_rb_tool['n'] == b_num]
                                        return row.iloc[0][p_type.lower()] if not row.empty else None

                                    c_pt1, c_pt2 = None, None
                                    if mm_mode == "1 Bar": c_pt1, c_pt2 = get_c_p(mm_b1, mm_a1), get_c_p(mm_b1, mm_a2)
                                    elif mm_mode == "2 Bars": c_pt1, c_pt2 = get_c_p(mm_b1, mm_a1), get_c_p(mm_b2, mm_a2)
                                    elif mm_mode == "Range":
                                        s_n, e_n = min(mm_b1, mm_b2), max(mm_b1, mm_b2)
                                        sub_df = c_rb_tool[(c_rb_tool['n'] >= s_n) & (c_rb_tool['n'] <= e_n)]
                                        if not sub_df.empty: c_pt1, c_pt2 = sub_df['high'].max(), sub_df['low'].min()

                                    if c_pt1 is not None and c_pt2 is not None:
                                        if mm_calc_type == "Fractions (%)":
                                            if c_pt1 == c_pt2: c_fig.add_hline(y=c_pt1, line_dash="solid", line_width=2, line_color="blue", annotation_text=" 0 / 1")
                                            else:
                                                for m in mm_levels_list:
                                                    lvl = c_pt1 + ((c_pt2 - c_pt1) * m)
                                                    c_fig.add_hline(y=lvl, line_dash=("solid" if m in [0,1] else "dash"), line_width=(2 if m in [0,1] else 1), line_color="blue", annotation_text=f" {m:g} ({lvl:.2f})")
                                        else:
                                            c_fig.add_hline(y=c_pt1, line_dash="solid", line_width=2, line_color="red", annotation_text=f" Base ({c_pt1:.2f})")
                                            for p in mm_pts_list:
                                                c_fig.add_hline(y=(c_pt1 + p), line_dash="solid", line_width=1, line_color="blue" if p>0 else "purple", annotation_text=f" {c_pt1+p:.2f} ({'+' if p>0 else ''}{p:g} pts)")

                                if sess == 'ETH' and pd.notna(c_info['on_h']) and pd.notna(c_info['on_l']):
                                    c_fig.add_shape(type="rect", x0=pd.to_datetime(f"{sd} 00:00:00"), y0=c_info['on_l'], x1=pd.to_datetime(f"{sd} {'08:00:00' if c_asset in ['DAX', 'FTSE'] else '12:00:00'}"), y1=c_info['on_h'], line=dict(color="RoyalBlue", width=1, dash="dot"), fillcolor="LightSkyBlue", opacity=0.15, layer="below")
                                if show_50_pct and not c_pd_df_slice.empty:
                                    c_fig.add_trace(go.Scatter(x=c_pd_df_slice['dt'], y=(c_pd_df_slice['high'] + c_pd_df_slice['low']) / 2, mode='markers', marker=dict(symbol='line-ew', size=14, color='rgba(255, 0, 0, 0.35)', line=dict(width=3, color='rgba(255, 0, 0, 0.35)')), showlegend=False, hoverinfo='skip'))
                                if pat != 'Any':
                                    c_t_bars, c_y_v, c_m_s = pd.DataFrame(), None, 'star'
        ...
        with perf_cont:
            ...
            with st.expander("📊 Strategy Performance Summary", expanded=False):
                if not df_f.empty:
                    ...
                    df_metrics['pts_diff'] = df_metrics['rc'] - df_metrics['ro'] if sess == 'RTH' else df_metrics['ec'] - df_metrics['eo']
                    df_metrics = df_metrics.dropna(subset=['pts_diff'])
                    samp_size = len(df_metrics)
                    if samp_size > 0:
                        wins, losses = df_metrics[df_metrics['pts_diff'] > 0], df_metrics[df_metrics['pts_diff'] <= 0]
                        win_ct, loss_ct = len(wins), len(losses)
                        ...
                        m2.metric("Win Rate", f"{(win_ct / samp_size) * 100:.1f}%")
                        m3.metric("Expectancy", f"{'+' if (tot_pl / samp_size)>0 else ''}{tot_pl / samp_size:.1f} pts")
                        m4.metric("Profit Factor", f"{(gr_prof / gr_loss):.2f}" if gr_loss != 0 else "MAX")

        with dates_cont:
            st.divider()
            st.markdown("### Jump to a Filtered Date")
            if pill_dates:
                css_rules = [f'''section[data-testid="stMain"] div[data-testid="stPills"] button:nth-child({i+1}) {{border-left: 6px solid {'#00c853' if df_f[df_f['Date'] == d].iloc[0]['dc'] == 'Green' else '#ff5252'} !important; border-top-left-radius: 4px !important; border-bottom-left-radius: 4px !important;}}''' for i, d in enumerate(pill_dates)]
